...
function ResponsiveDrawer(props) {
	...
	const buyRedirectToWhatsApp = () => {
		let pedidos = listCart
			.map(
				(item, index) =>
					` *${item.item.titulo}* x ${item.quantity} - valor: _R$ ${
						item.quantity * item.item.preco
					}_ `
			)
			.join();
	...
	const setCartFromResult = async (data, items) => {
		...
		for (let key in dictionaryWithCartItems) {
			mappedCartItems.push({
				id: key,
				quantity: data[1][key].quantity,
				item: items.find((element) => element.ID == key),
			});
		}
		...
		<div>
			...
			{listCategoria.map((item, index) => (
				...
				>
					<CardActionArea
						key={index}
						onClick={() => checked.indexOf(item) !== -1}
					>
						<CardContent>
						...
						>
							...
							>
								<Grid item xs={12} sm={6}>
									<Paper
										elevation={3}
										className={classes.numberButtonParent}
									>
										<div
											className={
												classes.numberGridContainer
											}
										>
											<div
												style={{
